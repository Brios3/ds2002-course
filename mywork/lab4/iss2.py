...
def load(df, csv_file='iss_results.csv'):
    if df is None:
    ...
    try:
        ...
        cursor.execute(
            ...
            (row['message'], row['latitude'], row['longitude'], row['timestamp'], REPORTER_ID)
        )
        db.commit()
    except Exception as e:
    ...
    try:
        ...
        logging.info(f"Wrote row to {csv_file}")
    except Exception as e:
